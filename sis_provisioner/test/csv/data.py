...
class CSVDataTest(TestCase):
    ...
    def test_sections(self):
        with self.settings(
            ...
            self.assertEquals(csv.add(formatter), False)
            self.assertEquals(csv.add(GroupSectionCSV('abc', 'active')), True)
    ...
    def test_enrollments(self):
        with self.settings(
            ...
            self.assertEquals(csv.add(EnrollmentCSV('abc', user, 'Student', 'active')), True)
            ...
            for registration in get_all_registrations_by_section(section):
                self.assertEquals(csv.add(StudentEnrollmentCSV(registration)), True)

            section = get_section_by_label('2013,spring,TRAIN,101/A')

            for user in section.get_instructors():
                self.assertEquals(csv.add(InstructorEnrollmentCSV(section, user, 'active')), True)
