...
urlpatterns = patterns('',
    ...
    url(r'api/v1/canvas/course/(?P<sis_id>[a-zA-Z0-9 &-]+)$', CanvasCourseView().run),
    url(r'api/v1/canvas/account/(?P<account_id>[0-9]+)$', CanvasAccountView().run),
    ...
    url(r'api/v1/uw/group/(?P<group_id>[a-zA-Z0-9\-\_\.]+)/membership$', GWSGroupMembers().run),
    url(r'api/v1/enrollments/?$', EnrollmentListView().run),
    url(r'api/v1/events/?(?P<begin>[0-9\-:TZtz]+)?(?P<end>[0-9\-:TZtz]+)?(?P<on>[0-9\-:TZtz]+)?(?P<type>\w+)?$', EventListView().run),
